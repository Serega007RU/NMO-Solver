...
const regexSymbols = /[^a-zA-Zа-яА-ЯёЁ0-9 .,!?'"\/\\+\-_×*%&@#;:(){}\[\]<>=|^~$№°⁰¹²³⁴⁵⁶⁷⁸⁹₽αβΔγμδσωτ→↑↓←≈]/gu // /u для .test а /gu для .replace
...
function normalizeTextNew(str, topic) {
    let text = str
        ...
        .replace(/[«»„“”″]/g, '"').replace(/[‘’‚‹›′`]/g, "'") // нестандартные кавычки
    ...
    if (forbiddenChars?.length) {
        ...
        if (newForbiddenChars?.length) {
            throw Error('Не удалось избавится от нестандартных символов ' + newForbiddenChars)
        }
    }
    ...
    if (!failedSuspiciousWords && (regexWords.test(text) || (text.match(regexWords)?.length))) {
        throw Error('Обнаружены подозрительные слова в тексте!')
    }
...
self.normalizeTextNew = normalizeTextNew