        ...
        if (message.question) {
            ...
            if (question) {
                ...
                // если найден и вопрос и к нему вариант ответов
                    ...
                    // если данный вариант ответов относится к другой теме, то явно прописываем id topic'а к данном варианту ответов
                    if (Object.keys(question.answers).length > 1 && (question.topics > 1 || question.topics[0] !== topic._id) && question.answers[answerHash].topics && !question.answers[answerHash].topics.includes(topic._id)) {
                ...
                // сохраняем правильный ответ или учитываем не правильный ответ
                    ...
                    if (resultQuestion.answers.usedAnswers?.length && resultQuestion.correct) {
                        ...
                        if (matchAnswers.length > 1) {
                            ...
                            // если данный вариант ответов относится к другой теме, то явно прописываем id topic'а к данном варианту ответов
                            if (Object.keys(question.answers).length > 1 && (question.topics > 1 || question.topics[0] !== topic._id) && question.answers[matchAnswers[0]].topics && !question.answers[matchAnswers[0]].topics.includes(topic._id)) {
                    ...
                    // если ответ не правильный, удаляем ту комбинацию (combination) которую мы использовали при подборе ответа
                        ...
                        if (matchAnswers.length > 1) {
                            ...
                            // если данный вариант ответов относится к другой теме, то явно прописываем id topic'а к данном варианту ответов
                            if (Object.keys(question.answers).length > 1 && (question.topics > 1 || question.topics[0] !== topic._id) && question.answers[matchAnswers[0].answerHash].topics && !question.answers[matchAnswers[0].answerHash].topics.includes(topic._id)) {
